...
#build a model secification
#combine into workflow
#fit a model to credit data
...
spec0<- boost_tree(mode='classification',
                   trees=200,
                   tree_depth=3,
                   learn_rate=0.15,
                   sample_size=0.65)%>%
...
one_row<- data.frame(Income=156,Seniority=5,Records='yes',Amount=4500,Job='fixed')
...
jsonlite::write_json(x=one_row,path='one_row')
